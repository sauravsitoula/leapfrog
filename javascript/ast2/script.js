...
function CarouselConstructor(container) {
    ...
    this.onClickPreviousButton = function() {
        ...
        var previousInterval = setInterval(function() {
            if (count === IMAGE_WIDTH) {
                var toSendForRemovingClassName =
                    Math.abs(IMAGE_WRAPPER_LEFT / 500) + "";
                clearInterval(previousInterval);
    ...
    this.autoSlideImages = function(setFirst) {
        console.log("INSIDE THE FUNCTION");
        if (NEXT_BUTTON_SET || PREVIOUS_BUTTON_SET || DOTS_SET) {
            return;
        }
        AUTO_SLIDE_SET = true;
        self = this;
        var count = 0;
        if (setFirst) {
            var beginningSlide = setInterval(function() {
                if (IMAGE_WRAPPER_LEFT === 0) {
                    clearInterval(beginningSlide);
                    AUTO_SLIDE_SET = false;
                    INDICATOR_DOTS[0].classList.add("active");
                    self.removeActiveStateFromDotClasses('0');
                    setTimeout(function() {
                        self.autoSlideImages(false);
                    }, 3000);
                    return;
                }
                IMAGE_WRAPPER_LEFT += 20;
                self.imageHolder.style.left = IMAGE_WRAPPER_LEFT + "px";
            }, 1);
            return;
        }
        var autoSlide = setInterval(function() {
            if (count === IMAGE_WIDTH) {
                ...
                INDICATOR_DOTS[toSendForRemovingClassName].classList.add("active");
                self.removeActiveStateFromDotClasses(toSendForRemovingClassName);
                clearTimeOutNextSlide = setTimeout(function() {
                    self.autoSlideImages(false);
                }, 3000);
                return;
            }
            if (IMAGE_WRAPPER_LEFT === (IMAGE_WRAPPER_WIDTH - IMAGE_WIDTH) * -1) {
                ...
                clearInterval(autoSlide);
                AUTO_SLIDE_SET = false;
                clearTimeOutFirstSlide = setTimeout(function() {
                    self.autoSlideImages(true);
                }, 0);
                return;
            }
            IMAGE_WRAPPER_LEFT -= 10;
